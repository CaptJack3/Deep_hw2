...
class LayerTrainer(Trainer):
    ...
    def train_batch(self, batch) -> BatchResult:
        ...
        # ====== YOUR CODE: ======
        class_scores = self.model(X) #. forward(X)
        loss =  self.loss_fn(class_scores,y) # maybe need .forward()
        self.optimizer.zero_grad()
        BB = self.model.backward(self.loss_fn.backward())
        self.optimizer.step()
        y_pred = torch.argmax(class_scores,dim=1)
        num_correct = torch.count_nonzero(y_pred == y)  # torch.count(y_pred==y)
    ...
    def test_batch(self, batch) -> BatchResult:
        ...
        class_scores = self.model(X)  # . forward(X)
        loss = self.loss_fn(class_scores, y)  # maybe need .forward()
        self.optimizer.zero_grad() # NEEDED ???
        y_pred = torch.argmax(class_scores, dim=1)
        num_correct = torch.count_nonzero(y_pred == y)  # torch.count(y_pred==y)
        # ========================
