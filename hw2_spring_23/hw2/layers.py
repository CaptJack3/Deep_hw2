...
class LeakyReLU(Layer):
    ...
    def forward(self, x, **kw):
        ...
        temp = self.alpha * x
        self.grad_cache["x"] = x
        out = torch.max(temp,x)
        # return torch.max(temp,x)
        # ========================
    ...
    def backward(self, dout):
        ...
        # ====== YOUR CODE: ======
        # temp = 1 * (x>0)
        # temp2 = self.alpha * (x<0)
        dout_dx = 1 * (x > 0)  + self.alpha * (x<0)

        # THIS IS THE CALC :  dL_dx = dL_dout * dout_dx
        dx = dout * dout_dx
        # ========================
...
class ReLU(LeakyReLU):
    ...
    def __init__(self):
        # ====== YOUR CODE: ======
        # FIXME JACK:  I'm not sure about this implementation !!!!!!!
        super().__init__()
        self.alpha = 0 # Special Case alpha == 0
        # ========================
...
class Sigmoid(Layer):
    ...
    def backward(self, dout):
        ...
        # ====== YOUR CODE: ======
        x=self.grad_cache["x"]
        grad_sigmoid = self.forward(x) * ( 1 - self.forward(x) )
        dx = dout * grad_sigmoid
        # ========================
        return dx
...
class Linear(Layer):
    ...
    def __init__(self, in_features, out_features, wstd=0.1):
        ...
        self.b = torch.zeros(size=(1,out_features)) # TOok me forever to decide if 1,D or D,1
        # ========================
    ...
    def forward(self, x, **kw):
        ...
        # ====== YOUR CODE: ======
        x=x.reshape((x.shape[0],-1)) # DIM OK?
        out = x @ self.w.T + self.b # Note: Diffrence between .t and .T ! .t expects input to be <2D
        # ========================
    ...
    def backward(self, dout):
        ...
        dx = dout @ self.w
        self.dw +=  torch.matmul(x.t() , dout).t()
        self.db +=   dout.sum(0)
        # ========================
...
class CrossEntropyLoss(Layer):
    ...
    def forward(self, x, y):
        ...
        # ====== YOUR CODE: ======
        temp=torch.sum(torch.exp(x),1)
        # print(x.size())
        import numpy as np
        temp2 = x[range(0,N),y].t()
        # print(y.size())
        # print(y)
        # print(N)
        # print(temp.size())
    ...
    def backward(self, dout=1.0):
        ...
        # ====== YOUR CODE: ======
        temp=torch.exp(x)
        temp1=torch.sum(temp,1).reshape((N,1))
        dx= (1/N) * temp/temp1
    ...
    def __init__(
        ...
        **kw,):
        ...
        for i in range(len(hidden_features)):
            ...
            layers.append(Linear(in_dim,hidden_features[i],wstd))
            if activation=='relu':
                layers.append(ReLU())
                if dropout != 0:
