...
class MomentumSGD(Optimizer):
    def __init__(self, params, learn_rate=1e-3, reg=0, momentum=0.9):
        ...
        # ====== YOUR CODE: ======
        # self.vel = [torch.zeros_like(param) for _,param in self.params]
        self.velocity =[torch.zeros_like(dp) for _,dp in params]
        # ========================

    def step(self):
        for (p,dp),vel in zip(self.params,self.velocity):
            if dp is None:
                continue
            ...
            # ====== YOUR CODE: ======
            dp += self.reg * p
            # self.v[dp] = self.momentum * self.v[dp] -self.learn_rate * dp
            # p += self.v[dp]
